...
def parse_args():
    ...
    parser.add_argument('--track', type=str, default='Nuerburgring',
    ...
    parser.add_argument('--max-lat-accel', type=float, default=MAX_LAT_ACCEL,
...
def equality_constraint(x, u, t, horizon, s_ref_end):
    ...
    return jnp.array([
        jnp.where(jnp.equal(t, horizon), s - s_ref_end, 0.0),  # End position constraint
        jnp.where(jnp.equal(t, horizon), ds_dt, 0.0),          # Zero final velocity
        jnp.where(jnp.equal(t, horizon), ds2_dt2, 0.0),        # Zero final acceleration
    ])
...
def inequality_constraint(x, u, t, s_ref, vx_ref, s_ref_end):
    ...
    long_accel_constraints = jnp.array([
        ds2_dt2 - MAX_LONGITUDINAL_ACCEL, 
        -ds2_dt2 - MAX_LONGITUDINAL_DECEL
    ])
...
def analyze_results(X, U, s_ref_end, kappa_actual, lateral_acceleration, dt):
    ...
    if max_long_accel > MAX_LONGITUDINAL_ACCEL + EPS:
        print(f"  WARNING: Longitudinal acceleration constraint violated by {max_long_accel - MAX_LONGITUDINAL_ACCEL:.2f} m/s²")
    if -max_long_decel > MAX_LONGITUDINAL_DECEL + EPS:
        print(f"  WARNING: Longitudinal deceleration constraint violated by {-max_long_decel - MAX_LONGITUDINAL_DECEL:.2f} m/s²")
...
def main():
    ...
    s_ref, kappa_ref, vx_ref, dt, N, horizon, x0, u0 = setup_optimization_problem(data, args)
    ...
    eq_constraint_fn = lambda x, u, t: equality_constraint(x, u, t, horizon, s_ref_end)
    ineq_constraint_fn = lambda x, u, t: inequality_constraint(x, u, t, s_ref, vx_ref, s_ref_end)
    
    print(f"Starting optimization with horizon: {horizon}, dt: {dt}")
    print(f"Track length: {s_ref_end:.2f} m")
    
    # Run optimization
    X, U, V, iteration_ilqr, iteration_al, no_errors = constrained_primal_dual_ilqr(
        cost_fn,
        dynamics_with_dt,
        x0,
        X_warm_start,
    ...
    print(f"Optimization completed: {iteration_ilqr=}, {iteration_al=}, {no_errors=}")
    ...
    # Plot results
    plot_speed_trajectory(X, U, f"{args.track}_speed_optimization_results", dt, lateral_acceleration)
